...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		pthread_mutex_init(&entry->mutex, NULL); // initialize a mutex for each bucket
	}
	return hash_table;
}
...
{
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	// lock the mutex for the bucket
	uint32_t errno = 0;
	errno = pthread_mutex_lock(&hash_table_entry->mutex);
	if (errno != 0){
		exit(errno);
	}
	...
	if (list_entry != NULL) {
		...
		errno = pthread_mutex_unlock(&hash_table_entry->mutex); // unlock before returning
		if(errno != 0){
			exit(errno);
		}
		return;
	...
	errno = pthread_mutex_unlock(&hash_table_entry->mutex);
		if(errno != 0){
			exit(errno);
		}
}
