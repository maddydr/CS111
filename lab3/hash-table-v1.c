...
struct list_entry { // structs the linked list points to
	const char *key;
	uint32_t value;
	SLIST_ENTRY(list_entry) pointers; 
};

SLIST_HEAD(list_head, list_entry); // defines the head of the linked list
...
{
	uint32_t errno = 0;
	//lock mutex as we are in the critical section
	...
	if (list_entry != NULL) {
		...
		errno = pthread_mutex_unlock(&hash_table->mutex); // unlock because leaving critical section
		if (errno != 0){
		exit(errno);
		}
		return;
