...
void write_superblock(int fd) {
	...
	superblock.s_r_blocks_count = 0;				/* done */
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS;	/* done */
	superblock.s_free_inodes_count = NUM_FREE_INODES;	/* done */
	superblock.s_first_data_block = SUPERBLOCK_BLOCKNO; /* done */	/* First Data Block */
	superblock.s_log_block_size = 0;				/* done */	/* 1024 */
	superblock.s_log_frag_size = 0;					/* done */	/* 1024 */
	superblock.s_blocks_per_group = BLOCK_SIZE * 8;	/* done */
	superblock.s_frags_per_group = BLOCK_SIZE * 8;	/* done */
	superblock.s_inodes_per_group = NUM_INODES;		/* done */
	superblock.s_mtime = 0;							/* done */ /* Mount time */
	superblock.s_wtime = current_time;				/* done */ /* Write time */
	superblock.s_mnt_count         = 0; 			/* done */ /* Number of times mounted so far */
	superblock.s_max_mnt_count     = -1; 			/* done */ /* Make this unlimited */
	superblock.s_magic = EXT2_SUPER_MAGIC; 			/* done */ /* ext2 Signature */
	superblock.s_state             = 1; 			/* done */ /* File system is clean */
	superblock.s_errors            = 1; 			/* done */ /* Ignore the error (continue on) */
	superblock.s_minor_rev_level   = 0; 			/* done */ /* Leave this as 0 */
	superblock.s_lastcheck = current_time; 			/* done */ /* Last check time */
	superblock.s_checkinterval     = 1; 			/* done */ /* Force checks by making them every 1 second */
	superblock.s_creator_os        = 0; 			/* done */ /* Linux */
	superblock.s_rev_level         = 0; 			/* done */ /* Leave this as 0 */
	superblock.s_def_resuid        = 0; 			/* done */ /* root */
	superblock.s_def_resgid        = 0; 			/* done */ /* root */
...
void set_bit(u8 *bitmap, u32 index){
	u32 byte_index = index / 8; // find byte index for next byte
	u32 bit_position = index % 8;
	u8 bitmask = 1 << bit_position;
	bitmap[byte_index] = bitmap[byte_index] | bitmask; // set bit with OR 
}
...
void write_block_bitmap(int fd)
{
	...
	// TODO It's all yours 		/* done */
	// anything less than block 24 is taken -- blocks 24-1023 free
	u8 map_value[BLOCK_SIZE] = {0}; // 1024 entries, each 8 bits long, initialize as free
	// tried bit manipulation, doing brute froce
	map_value[0] = 0xFF; // 8 1's
	map_value[1] = 0xFF; // 8 1's
	map_value[2] = 0x7F; // 7 1's -- 8+8+7=23
	map_value[127] = 0x80; // remainder
...
void write_inode_bitmap(int fd)
{
	...
	// TODO It's all yours 	/* done */
	u8 map_value[BLOCK_SIZE] = {0}; // want 14th and onward to be free
...
void write_inode_table(int fd) {
	u32 current_time = get_current_time();
	// this does the lost and found inode permissions
	...
	root_directory_inode.i_mode = EXT2_S_IFDIR | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IXUSR | EXT2_S_IRGRP | EXT2_S_IXGRP | EXT2_S_IROTH | EXT2_S_IXOTH;
	...
	root_directory_inode.i_links_count = 3; // the . .. and then a link to itself
...
void write_root_dir_block(int fd)
{
	...
	space_left -= parent_entry.rec_len;

	// for lost and found dir
	...
	space_left -= lost_and_found.rec_len;
	...
	space_left -= hello_world_entry.rec_len;
	...
	space_left -= hello_entry.rec_len;

	// padding
	struct ext2_dir_entry fill_entry = {0};
	fill_entry.rec_len = space_left;
